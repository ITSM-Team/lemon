				...
				<div class="panel panel-default">
					<div class="panel panel-default">
						<div class="panel-heading">
							<i class="glyphicon glyphicon-list"></i> 组织结构
							<div class="pull-right ctrl">
								<a class="btn btn-default btn-xs"><i id="orgSearchIcon1" class="glyphicon glyphicon-chevron-up"></i></a>
							</div>
						</div>
						<div class="panel-body">
							<%@include file="/menu/org.jsp"%>
							<form name="orgForm" method="post" action="org-list.do" class="form-inline">
								<input type="hidden" name="partyStructTypeId" value="${param.partyStructTypeId}"> <input type="hidden" name="partyEntityId" value="${param.partyEntityId}"> <label for="org_name"><spring:message code='org.org.list.search.name' text='名称' />:</label> <input type="text" id="org_name" name="name" value="${param.name}" class="form-control">
								...
								<tbody>
									<c:forEach items="${page.result}" var="item">
										<tr>
											...
											<td>
												<a href="org-remove.do?selectedItem=${item.id}&partyStructTypeId=${partyStructTypeId}&partyEntityId=${partyEntityId}" class="a-remove">删除</a> <c:if test="${item.childEntity.partyType.type==1}">
