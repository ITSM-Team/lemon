...
public class PartyOrgConnector implements OrgConnector {
	...
	public int getJobLevelByUserId(String userId) {
		...
		// if (partyStruct.getParentEntity().getPartyType().getType() ==
		...
		for (PartyStruct partyStruct : partyEntity.getChildStructs()) {
			if ("user-position".equals(partyStruct.getPartyStructType().getType())) {
	...
	public String getSuperiorId(String userId) {
		...
		if (superior == null) {
			logger.info("cannot find superiour : {} {}", partyEntity.getName(), partyEntity.getId());
		...
		while (partyEntity != null) {
			...
			for (PartyStruct partyStruct : partyEntity.getChildStructs()) {
				...
				if (childPartyEntity.getPartyType().getType() == PartyConstants.TYPE_USER) {
					...
					List<PartyEntity> users = this.findByPosition(partyEntity, childPartyEntity.getName());
	 ...
	 * 在本部门下，查找对应职位的人员.
	 */
	public List<PartyEntity> findByPosition(PartyEntity partyEntity, String positionName) {
	...
	public boolean isAdmin(PartyStruct partyStruct) {
		...
		logger.info("department : {} {}", department.getName(), department.getId());

		// 遍历上级部门的每个叶子
		for (PartyStruct childPartyStruct : department.getChildStructs()) {
			if (!"manage".equals(childPartyStruct.getPartyStructType().getType())) {
				continue;
			}
			...
			if (childPartyEntity.getPartyType().getType() == PartyConstants.TYPE_USER) {
				...
				// 如果是岗位，继续查找部门下所有岗位对应的人员，返回
				List<PartyEntity> users = this.findByPosition(department, childPartyEntity.getName());
	...
	public List<String> findPositionUserIds(PartyEntity parent, String positionName) {
		...
		while (partyEntity != null) {
			...
			/*
			 * else if ((parent.getPartyType().getType() ==
			 * PartyConstants.TYPE_POSITION) &&
			 * parent.getName().equals(positionName)) { //
			 * 如果parent已经是岗位了，而且名字与期望的positionName一致 for (PartyStruct
			 * partyStruct : parent.getChildStructs()) { PartyEntity child =
			 * partyStruct.getChildEntity();
			 * 
			 * // 就把岗位下的人直接附加到userIds里 if (child.getPartyType().getType() ==
			 * PartyConstants.TYPE_USER) { userIds.add(child.getRef()); } } }
			 */
			if (userIds.isEmpty()) {
	 ...
	 * 获得上级部门.
	 */
	public PartyEntity findUpperDepartment(PartyEntity child, boolean skipAdminDepartment) {
		if (child == null) {
		...
		for (PartyStruct partyStruct : child.getParentStructs()) {
			...
			if (parent.getPartyType().getType() == PartyConstants.TYPE_ORG) {
				if (skipAdminDepartment && this.isAdmin(partyStruct)) {
					return this.findUpperDepartment(parent, skipAdminDepartment);
				} else {
