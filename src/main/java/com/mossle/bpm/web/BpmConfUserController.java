...
//    @Autowired
//    private BpmConfUserMongoDao bpmConfUserMongoDao;
//    @Autowired
//    private BpmConfCountersignMongoDao bpmConfCountersignMongoDao;
//    @Autowired
//    private BpmConfAssignMongoDao bpmConfAssignMongoDao;
...
//        List<BpmConfUser> bpmConfUsers =  bpmConfUserMongoDao.find(query);
        BpmConfCountersign bpmConfCountersign = bpmConfCountersignManager
                .findUniqueBy("bpmConfNode", bpmConfNode);
        //BpmConfCountersign bpmConfCountersign = bpmConfCountersignMongoDao.findOne(query);
        
        BpmConfAssign bpmConfAssign = bpmConfAssignManager.findUniqueBy(
                "bpmConfNode", bpmConfNode);
//        BpmConfAssign bpmConfAssign =  bpmConfAssignMongoDao.findOne(query);
    ...
    public String save(@ModelAttribute BpmConfUser bpmConfUser,
        ...
        if (bpmConfUser.getType() == 0) {
            ...
            BpmConfUser targetBpmConfUser = bpmConfUserManager.findUnique(hql,
                    bpmConfNodeId);        	
//            Query query=new Query();
//            query.addCriteria(Criteria.where("bpmConfNode.id").is(bpmConfNodeId)
//            		.and("type").is(0)
//            		.and("status").is(0));
//            BpmConfUser targetBpmConfUser = bpmConfUserMongoDao.findOne(query);
           
            if (targetBpmConfUser != null) {
                targetBpmConfUser.setStatus(2);
               bpmConfUserManager.save(targetBpmConfUser);
               // bpmConfUserMongoDao.save(targetBpmConfUser);
            }
        }
...
//        }
//        bpmConfUserMongoDao.save(bpmConfUser);
    ...
    public String remove(@RequestParam("id") Long id) {
       ...
       // BpmConfUser bpmConfUser =bpmConfUserMongoDao.findById(id);
        ...
        if (bpmConfUser.getStatus() == 0) {
            ...
            //  bpmConfUserMongoDao.remove(bpmConfUser.getId());
