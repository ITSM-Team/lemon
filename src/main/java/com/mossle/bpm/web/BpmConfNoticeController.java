...
//    @Autowired
//    private BpmConfNoticeMongoDao bpmConfNoticeMongoDao;
//    @Autowired
//    private BpmMailTemplateMongoDao bpmMailTemplateMongoDao;
...
//        List<BpmConfNotice> bpmConfNotices = bpmConfNoticeMongoDao.find(query);
        List<BpmMailTemplate> bpmMailTemplates = bpmMailTemplateManager
                .getAll();
      //  List<BpmMailTemplate> bpmMailTemplates =   bpmMailTemplateMongoDao.findAll();
    ...
    //  bpmConfNoticeMongoDao.save(bpmConfNotice);
    ...
    public String remove(@RequestParam("id") Long id) {
      ...
      //BpmConfNotice bpmConfNotice =bpmConfNoticeMongoDao.findById(id);
        Long bpmConfNodeId = bpmConfNotice.getBpmConfNode().getId();
        bpmConfNoticeManager.remove(bpmConfNotice);
        // bpmConfNoticeMongoDao.remove(bpmConfNotice.getId());
        BpmConfNotice dest = bpmConfNotice;
        String taskDefinitionKey = dest.getBpmConfNode().getCode();
        String processDefinitionId = dest.getBpmConfNode().getBpmConfBase()
                .getProcessDefinitionId();
