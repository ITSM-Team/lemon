...
//    private TaskParticipantMongoDao taskParticipantMongoDao;
    ...
    public void removeHumanTask(String humanTaskId) {
        ...
        TaskInfo taskInfo =  taskInfoMongoDao.findById(Long.parseLong(humanTaskId));
    ...
    public void removeHumanTaskByTaskId(String taskId) {
        ...
        TaskInfo taskInfo =  taskInfoMongoDao.findOne(query);
        this.removeHumanTask(taskInfo);
    }
...
//        List<TaskInfo> taskInfos = taskInfoManager.find(hql, processInstanceId);
        ...
        List<TaskInfo> taskInfos =taskInfoMongoDao.find(query);
    ...
    public void removeHumanTask(TaskInfo taskInfo) {
        ...
        taskInfoMongoDao.remove(taskInfo.getId());
    }
    ...
    public HumanTaskDTO saveHumanTask(HumanTaskDTO humanTaskDto,
        ...
        if (id != null) {
            ...
            taskInfo = taskInfoMongoDao.findById(id);
        }
        ...
        if (humanTaskDto.getParentId() != null) {
        	 taskInfo.setTaskInfo(taskInfoMongoDao.findById(Long
...
//                    .parseLong(humanTaskDto.getParentId())));
        ...
        if(taskInfoID==null){
        	taskInfo.setId(idGenerator.generateId());
        }
       // taskInfoMongoDao.save(taskInfo);
        if (triggerListener) {
            // create
    ...
    public HumanTaskDTO findHumanTaskByTaskId(String taskId) {
     //  TaskInfo taskInfo = taskInfoManager.findUniqueBy("taskId", taskId);
       TaskInfo taskInfo = taskInfoMongoDao.findOne(
...
//                        processInstanceId);
        ...
        .with(new Sort(Direction.ASC,"createTime"));
        List<TaskInfo> taskInfos =taskInfoMongoDao.find(query);
...
//                Long.parseLong(parentTaskId));
        List<TaskInfo> taskInfos =  taskInfoMongoDao.find(Query.query(Criteria.where("taskInfo.id")
        		.is(Long.parseLong(parentTaskId))));
        return this.convertHumanTaskDtos(taskInfos);
    }
...
//                "from TaskDeadline where taskInfo.id=?", longTaskId);
        ...
        List<TaskDeadline> taskDeadlines =taskDeadlineMongDao.find(
        		Query.query(Criteria.where("taskInfo.id").is(longTaskId)));
        
        
        for (TaskDeadline taskDeadline : taskDeadlines) {
            //taskDeadlineManager.remove(taskDeadline);
        	taskDeadlineMongDao.remove(taskDeadline.getId());
        }
        ...
        if (humanTaskListeners != null) {
           ...
           TaskInfo taskInfo =taskInfoMongoDao.findById(id);
...
//                        pageNo, pageSize, userId, tenantId);
    	Page page=new Page();
        Query query=new Query();
        query.addCriteria((Criteria.where("assignee").is(userId)
        		.and("tenantId").is(tenantId)
        		.and("status").is("active")));
        page.setPageNo(pageNo);
        page.setPageSize(pageSize);
        page=taskInfoMongoDao.findPage(page,new ArrayList<PropertyFilter>() , query);
...
//                        pageNo, pageSize, userId, tenantId);
    ...
    public void claimTask(String humanTaskId, String userId) {
       ...
       TaskInfo taskInfo = taskInfoMongoDao.findById(Long.valueOf(humanTaskId));
